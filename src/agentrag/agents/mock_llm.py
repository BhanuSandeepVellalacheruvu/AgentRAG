...
class MockChatBedrock:
    ...
    def invoke(self, messages: Any) -> MockResponse:
        ...
        # If context was retrieved from the local FAISS/BM25 store, extract the exact answer from the top chunk!
        ...
        if "compensatory" in user_query_lower or "comp off" in user_query_lower:
            content = (
                "When applying for compensatory off (comp off), your email to your manager should include:\n"
                ...
                "4. A reference to the prior approval received for the overtime work."
            )
        elif "unethical" in user_query_lower or "ethics" in user_query_lower or "conduct" in user_query_lower:
            content = (
                "For ethical concerns or questions about unethical behavior, you should consult the company's "
                "Code of Conduct and Ethics Policy, or reach out to the compliance department. Keeping "
                "workplace integrity high is our priority, and all reports can be submitted confidentially."
            )
        elif "review" in user_query_lower or "annual" in user_query_lower or "why" in user_query_lower:
            content = (
                "The company reviews its policies annually to ensure alignment with local labor regulations, "
                "incorporate feedback from the teams, and maintain competitive standards for the organization."
            )
            ...
            content = "Sick leave covers up to 10 paid days per year for medical recovery. Absences over 3 consecutive days require a doctor's note."
        elif "leave" in user_query_lower or "policy" in user_query_lower:
            content = (
                "All standard company policies (including leave, code of conduct, and remote work) are "
                "documented in the Employee Handbook. Let me know if you want information on a specific policy!"
            )
        else:
        ...
        class MockStructuredRunnable:
            def invoke(self, messages: Any) -> Any:
                ...
                if schema_name == "RouteDecision":
                    # Route to retrieve if they ask about policy, vacation, leave, sick, comp off, ethics, conduct, bribery, or report
                    keywords = [
                        "policy", "hr", "vacation", "leave", "sick", "medical", "compensatory", 
                        "comp off", "off", "ethics", "conduct", "bribe", "bribery", "kreeda", 
                        "labs", "report", "contact", "whistleblower", "violation", "contractor"
                    ]
