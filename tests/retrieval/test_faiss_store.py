...
def test_add_chunks_and_search(temp_store_dir: Path) -> None:
    ...
    store = HybridFAISSStore(
        embedding_provider=provider, dimension=5, persist_dir=temp_store_dir
    )  # noqa: E501
    ...
    chunks = [
        ...
        Chunk(
            text="apple orange kiwi", filename="3.txt", source="3.txt", chunk_index=0
        ),  # noqa: E501
    ]
    ...
    empty_store = HybridFAISSStore(
        embedding_provider=provider,
        dimension=5,
        persist_dir=temp_store_dir.parent / "empty",
    )  # noqa: E501
...
def test_load_from_s3_success(
    mock_boto3_client: MagicMock, temp_store_dir: Path
) -> None:  # noqa: E501
    ...
    mock_ddb.get_item.return_value = {"Item": {"sha256_hash": {"S": valid_hash}}}

    # Load into new dir
    new_dir = temp_store_dir.parent / "new_store"
    new_store = HybridFAISSStore(
        embedding_provider=provider, dimension=5, persist_dir=new_dir
    )  # noqa: E501
...
@patch("boto3.client")
def test_load_from_s3_integrity_failure(
    mock_boto3_client: MagicMock, temp_store_dir: Path
) -> None:  # noqa: E501
    ...
    mock_s3.get_object.return_value = {
        "Body": MagicMock(read=MagicMock(return_value=b"fake data"))
    }  # noqa: E501
    mock_ddb.get_item.return_value = {"Item": {"sha256_hash": {"S": "wrong_hash"}}}
